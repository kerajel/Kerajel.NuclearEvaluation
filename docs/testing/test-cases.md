...
| ADD-42 | Backend/API | Chart APIs accept Radzen nullable comparison filters | POST APM and particle chart APIs with `x => ((x.U234 ?? null) > 1)` | Endpoints return HTTP 200 with chart arrays instead of a dynamic-LINQ comparison error. | PASS | Chart APIs accepted nullable comparison filters. |
| ADD-43 | Backend/API | Preset create accepts enabled entries with null navigation payloads | POST a preset filter with enabled sample descriptors and a null navigation property | Endpoint creates the preset and returns it with its entry payload. | PASS | Preset create returned the enabled sample entry payload and cleanup removed the test preset. |
