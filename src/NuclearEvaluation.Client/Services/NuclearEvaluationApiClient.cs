...
public class NuclearEvaluationApiClient : INuclearEvaluationApi
{
    ...
    public Task<DataResult<SeriesView>> GetSeriesViews(
        DataQuery query,
        CancellationToken ct = default
    ) => PostQuery<SeriesView>("api/views/series", query, ct);

    public Task<DataResult<SampleView>> GetSampleViews(
        DataQuery query,
        CancellationToken ct = default
    ) => PostQuery<SampleView>("api/views/samples", query, ct);

    public Task<DataResult<SubSampleView>> GetSubSampleViews(
        DataQuery query,
        CancellationToken ct = default
    ...
    public Task<DataResult<ParticleView>> GetParticleViews(
        DataQuery query,
        CancellationToken ct = default
    ) => PostQuery<ParticleView>("api/views/particles", query, ct);

    public Task<DataResult<ProjectView>> GetProjectViews(
        DataQuery query,
        CancellationToken ct = default
    ) => PostQuery<ProjectView>("api/views/projects", query, ct);

    public Task<DataResult<StemPreviewEntryView>> GetStemPreviewEntryViews(
        DataQuery query,
        CancellationToken ct = default
    ...
    public async Task<SeriesCountsView> GetSeriesCounts(
        DataQuery query,
        CancellationToken ct = default
    )
    {
        using HttpResponseMessage response = await _http.PostAsJsonAsync(
            "api/views/series-counts",
            query,
            JsonOptions,
            ct
        );
        response.EnsureSuccessStatusCode();
        return await response.Content.ReadFromJsonAsync<SeriesCountsView>(JsonOptions, ct)
            ?? new SeriesCountsView();
    }

    public async Task<List<int>> GetEnumFilterOptions(
        string entity,
    ...
    public async Task<List<IsotopeBinCounts>> GetProjectApmUraniumBinCounts(
        DataQuery query,
        CancellationToken ct = default
    )
    {
        using HttpResponseMessage response = await _http.PostAsJsonAsync(
            "api/charts/apm-bin-counts",
            query,
            JsonOptions,
            ct
        );
    ...
    public async Task<List<IsotopeBinCounts>> GetProjectParticleUraniumBinCounts(
        DataQuery query,
        CancellationToken ct = default
    )
    {
        using HttpResponseMessage response = await _http.PostAsJsonAsync(
            "api/charts/particle-bin-counts",
            query,
            JsonOptions,
            ct
        );
    ...
    {
        using HttpRequestMessage request = new(HttpMethod.Delete, "api/series")
        {
    ...
    public async Task<List<SampleView>> GetSamplesForSeries(
        int seriesId,
        CancellationToken ct = default
    ) =>
        await _http.GetFromJsonAsync<List<SampleView>>(
            $"api/series/{seriesId}/samples",
            JsonOptions,
            ct
        ) ?? [];
    ...
    public async Task UpdateProjectSeries(
        ProjectSeriesUpdate update,
        CancellationToken ct = default
    )
    {
        using HttpResponseMessage response = await _http.PostAsJsonAsync(
    ...
    public async Task DeletePresetFilter(int id, CancellationToken ct = default)
    {
        using HttpResponseMessage response = await _http.DeleteAsync(
            $"api/preset-filters/{id}",
            ct
        );
        response.EnsureSuccessStatusCode();
    }

    public async Task<bool> IsProjectNameAvailable(
        string name,
        int excludeId,
        CancellationToken ct = default
    ) =>
        await _http.GetFromJsonAsync<bool>(
            $"api/projects/name-available?name={Uri.EscapeDataString(name)}&excludeId={excludeId}",
            JsonOptions,
            ct
        );

    public async Task<bool> IsPresetFilterNameAvailable(
        string name,
        int excludeId,
        CancellationToken ct = default
    ) =>
        await _http.GetFromJsonAsync<bool>(
            $"api/preset-filters/name-available?name={Uri.EscapeDataString(name)}&excludeId={excludeId}",
            JsonOptions,
            ct
        );

    public async Task<OperationOutcome> UploadStemPreviewFile(
        Guid sessionId,
        Guid fileId,
        string fileName,
        Stream content,
    ...
    )
    {
        ...
        response.EnsureSuccessStatusCode();
        return await response.Content.ReadFromJsonAsync<OperationOutcome>(JsonOptions, ct)
            ?? OperationOutcome.Fail("Empty response.");
    }

    public async Task DeleteStemPreviewFile(
        Guid sessionId,
        Guid fileId,
        CancellationToken ct = default
    )
    {
        using HttpResponseMessage response = await _http.DeleteAsync(
            $"api/stem/{sessionId}/files/{fileId}",
            ct
        );
        response.EnsureSuccessStatusCode();
    }
    ...
    public async Task<CaptchaStatus> GetCaptchaStatus(CancellationToken ct = default) =>
        await _http.GetFromJsonAsync<CaptchaStatus>("api/captcha/status", JsonOptions, ct)
        ?? new CaptchaStatus();

    public async Task<CaptchaChallenge> GetCaptchaChallenge(CancellationToken ct = default) =>
        await _http.GetFromJsonAsync<CaptchaChallenge>("api/captcha/challenge", JsonOptions, ct)
        ?? new CaptchaChallenge();

    public async Task<CaptchaStatus> VerifyCaptcha(
    ...
    )
    {
        ...
        if (response.StatusCode == System.Net.HttpStatusCode.BadRequest)
            return new CaptchaStatus();
        response.EnsureSuccessStatusCode();
        return await response.Content.ReadFromJsonAsync<CaptchaStatus>(JsonOptions, ct)
            ?? new CaptchaStatus();
    }
}
