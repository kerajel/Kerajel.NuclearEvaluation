...
public partial class ProjectApmUraniumBinCountsChart
{
    ...
    ILookup<string, BinCount> _apmUraniumBinCounts = Enumerable
    ...
    async Task Load(DataQuery query)
    {
        ...
        try
        {
            data = await Api.GetProjectApmUraniumBinCounts(query);
        }
        ...
        _apmUraniumBinCounts = data.SelectMany(x => x.Bins.Select(b => (x.Isotope, Bin: b)))
