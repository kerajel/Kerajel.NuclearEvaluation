...
{
    public static FetchDataCommand<T> ToCommand<T>(
        this DataQuery query,
        CancellationToken ct = default
    )
        where T : class
    {
        return new FetchDataCommand<T> { Query = query, CancellationToken = ct };
    }
    ...
    public static DataResult<T> ToDataResult<T>(this FetchDataResult<T> result)
    {
        ...
        {
            return DataResult<T>.Faulted("Could not load the requested data. Please try again.");
        }
